...
sample_skew     <- skewness(log_returns_vector)
sample_kurtosis <- kurtosis(log_returns_vector)

cat("Sample Skewness :", round(sample_skew, 4), "\n")
...
ggplot(data.frame(Returns = log_returns_vector), aes(x = Returns)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "deepskyblue3", color = "black", alpha = 1) +
...
# Load Bond Data
file_path <- "/Users/fernandoperezmillan/Desktop/Tesis/final_bond_data.xlsx"
...
ggplot(rolling_cov_df, aes(x = Date, y = Correlation)) +
  geom_rect(aes(xmin = as.Date("2020-01-01"), xmax = as.Date("2020-12-31"),
                ymin = -0.0005, ymax = 0.00005),
            fill = "palegreen1", alpha = 0.015, color = "seagreen4", linewidth = 0.6,
            inherit.aes = FALSE) +
...
ggplot(garch_df, aes(x = Date)) +
  ...
  scale_x_date(date_breaks = "2 year", date_labels = "%Y") +labs(
       x ="",
       y = "Variance") +
  theme_tufte(base_family = "Times") + 
  theme(panel.grid = element_blank(),  
...
ggplot(df_var, aes(x = Date)) +
  ...
  geom_line(aes(y = VaR_025_GARCH, color = "VaR (2.5%)"), linetype = "dashed", linewidth = 1) +
  geom_point(
    data = subset(df_var, Breach_GARCH == 1),
    aes(y = Returns, shape = "VaR Breach"), color = "blue", fill = "white", size = 2, stroke = 1
  ) +
  scale_color_manual(values = c("Actual Return" = "black", "VaR (2.5%)" = "deepskyblue3")) +
  scale_shape_manual(values = c("VaR Breach" = 21)) +
  labs(
    x = "Date", y = "Log return", color = "Legend", shape = ""
  ) +
  theme_tufte(base_family = "Times") +
  theme(
    panel.grid = element_blank(),
...
ggplot(df_var, aes(x = Date)) +
  geom_line(aes(y = Returns, color = "Actual Return")) +
  geom_line(aes(y = VaR_025_GJR, color = "VaR (2.5%)"), linetype = "dashed", linewidth = 1) +
  geom_point(
    data = subset(df_var, Breach_GJR == 1),
    aes(y = Returns, shape = "VaR Breach"),
    color = "green4", fill = "white", size = 2, stroke = 1
  ) +
  scale_color_manual(
    values = c(
      "Actual Return" = "black",
  ...
  scale_shape_manual(values = c("VaR Breach" = 21)) +
  guides(
    shape = guide_legend(order = 1),  # VaR Breach first
    color = guide_legend(order = 2)   # Lines second
  ) +
  labs(
    x = "Date", y = "Log return", color = "Legend", shape = ""
  ) +
...
ggplot(df_var, aes(x = Date)) +
  geom_line(aes(y = Returns, color = "Actual Return"), linewidth = 0.5) +
  geom_line(aes(y = VaR_025_GARCH, color = "VaR (2.5%)"), 
            linetype = "dashed", linewidth = 1) +
  geom_line(aes(y = ES_025_GARCH, color = "ES (2.5%)"), 
            linetype = "solid", linewidth = 0.7) +
  geom_point(
    data = subset(df_var, Breach_GARCH == 1),
    aes(y = Returns, shape = "VaR Breach"),
    color = "blue", fill = "white", size = 2, stroke = 1
  ) +
  scale_color_manual(values = c(
    "Actual Return" = "black",
    "VaR (2.5%)" = "deepskyblue3",
    "ES (2.5%)" = "deepskyblue3"
  )) +
  scale_shape_manual(values = c("VaR Breach" = 21)) +
  labs(
...
ggplot(df_var, aes(x = Date)) +
  geom_line(aes(y = Returns, color = "Actual Return"), linewidth = 0.5) +
  geom_line(aes(y = VaR_025_GJR, color = "VaR (2.5%)"), 
            linetype = "dashed", linewidth = 1) +
  geom_line(aes(y = ES_025_GJR, color = "ES (2.5%)"), 
            linetype = "solid", linewidth = 0.7) +
  geom_point(
    data = subset(df_var, Breach_GJR == 1),
    aes(y = Returns, shape = "VaR Breach"),
...
ggplot(df_var, aes(x = Date)) +
  geom_line(aes(y = Returns, color = "Actual Return")) +
  geom_line(aes(y = VaR_EVT_GARCH, color = "VaR (2.5%)"), linetype = "dashed", linewidth = 1) +
  geom_line(aes(y = ES_EVT_GARCH, color = "ES (2.5%)"), linetype = "solid", linewidth = 1) +
  geom_point(
    data = subset(df_var, Breach_EVT_GARCH == 1),
    aes(y = Returns, shape = "VaR Breach"),
    color = "red", fill = "white", size = 2, stroke = 1
  ) +
  scale_color_manual(
    values = c(
      "Actual Return" = "black",
...
ggplot(df_var, aes(x = Date)) +
  geom_line(aes(y = Returns, color = "Actual Return")) +
  geom_line(aes(y = VaR_EVT_GJR, color = "VaR (2.5%)"), linetype = "dashed", linewidth = 1) +
  geom_line(aes(y = ES_EVT_GJR, color = "ES (2.5%)"), linetype = "solid", linewidth = 1) +
  geom_point(
    data = subset(df_var, Breach_EVT_GJR == 1),
    aes(y = Returns, shape = "VaR Breach"),
    color = "purple", fill = "white", size = 2, stroke = 1
  ) +
  scale_color_manual(
    values = c(
      "Actual Return" = "black",
